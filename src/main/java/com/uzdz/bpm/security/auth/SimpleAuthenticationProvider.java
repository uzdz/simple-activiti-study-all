...
public class SimpleAuthenticationProvider implements AuthenticationProvider {
     ...
     * SimpleLoginToken#SimpleLoginToken(Object, Object) 方法生成的,是未认证状态的(setAuthenticated(false))
    ...
    public Authentication authenticate(Authentication authentication) throws AuthenticationException {
        ...
        SimpleLoginToken token= (SimpleLoginToken) authentication;
        //检查
        defaultCheck(userDetails);
        additionalAuthenticationChecks(userDetails, token);
        //构造已认证的authentication
        SimpleLoginToken authenticatedToken = new SimpleLoginToken(userDetails, token.getCredentials(), userDetails.getAuthorities());
    ...
    public boolean supports(Class<?> authentication) {
        return (SimpleLoginToken.class
                .isAssignableFrom(authentication));
    }

    /**(附加检查项)检查密码是否正确*/
    private void additionalAuthenticationChecks(UserDetails userDetails,
                                                SimpleLoginToken authentication) throws AuthenticationException {
